...
public class SecurityConfig {
    ...
    protected SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.authorizeRequests() // 각 경로에 따른 권한 지정
                .antMatchers("/", "/login", "/register", "/index", "/css/**", "/fonts/**", "/img/**", "/js/**", "/scripts/**", "/scss/**", "/src/**", "/gallery/**", "/ajax-gallery/**", "/board", "/board/content/**","/login/**").permitAll() // 누구나 가능
                .antMatchers(HttpMethod.POST, "/admin/regist", "/admin/login", "/board/**", "/ajax/replies/**", "/update","/update-kakao","userDelete").permitAll()
                .antMatchers("/board/**","/mypage/**").hasAnyRole("ADMIN","USER","KAKAO") // "admin"만 접근 가능, 테이블에 ROLE_권한명 으로 저장해야함
