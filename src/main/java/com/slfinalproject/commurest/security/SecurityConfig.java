...
public class SecurityConfig {
    ...
    protected SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.authorizeRequests() // 각 경로에 따른 권한 지정
                .antMatchers("/", "/login", "/register", "/index", "/css/**", "/fonts/**", "/img/**", "/js/**", "/scripts/**", "/scss/**", "/src/**", "/gallery/**", "/ajax-gallery/**", "/board", "/board/content/**","/ajax/replies/**","/login/**","/search/**", "/loadFile/**", "/board/file/**","/ajax/recommend/**").permitAll() // 누구나 가능
                .antMatchers(HttpMethod.POST,"/upload","/ajax-upload", "/admin/regist", "/admin/login", "/board/**", "/ajax/replies/**", "/update","/update-kakao","userDelete").permitAll()
